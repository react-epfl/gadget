myApp.controller('WebSocketController', ['$scope', function($scope) {
    ...
    function getScale() {
        ...
        $(".gadget4 #piezoFrequencyDiv").css({'zoom': (w/796)});
        if (FF) {
            $('#piezoVoltageDiv').css({MozTransform: 'scale(' + (w/796) + ')'});
            $('#piezoFrequencyDiv').css({MozTransform: 'scale(' + (w/796) + ')'});
        }
        var wScale = w / 900;
        var hScale = h / 500;
                        ...
                        if (msg.sensorId != "Info") {
                            ...
                            if (msg.responseData) {
                                ...
                                if (Q_Size == 0) {
                                    if (!controller) {
                                        setTimeout (function() {
                                            configureSD($scope.laserIsOn, bs1IsActivated, bs2IsActivated, [parseFloat(piezoVoltageSlider.noUiSlider.get()[0]),1, parseFloat(piezoFrequencySlider.noUiSlider.get()[0])], [filterIsActivated, enableCamera1]);
                                        }, 700);
                                        controller = 1;
                                    }
                                }
    ...
    var initiatePiezoMovement = function() {
        if (piezoInterval) clearInterval(piezoInterval);
        if (piezoInterval) clearInterval(piezoInterval);
        $(".gadget4 #miroirNoir2").velocity('stop').velocity('stop').velocity('stop').velocity('stop').velocity({translateY: 0 + "px", translateX: + 0 + "px"}, {duration: 0});
        $(".straight-line-bottom").velocity('stop').velocity('stop').velocity('stop').velocity('stop').velocity({translateY: 0 + "px"}, {duration: 0});
        $(".straight-line-bottom2").velocity('stop').velocity('stop').velocity('stop').velocity('stop').velocity({translateY: 0 + "px", translateX: 0 + "px"}, {duration: 0});
        $(".gadget4 #miroirNoir2").velocity({translateY: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px"}, {duration: periodValue / 2});
        $(".straight-line-bottom").velocity({translateY: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px"}, {duration: periodValue / 2});
        $(".straight-line-bottom2").velocity({translateY: (-1) * (translateValue) + "px", translateX: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px", translateX: 0 + "px"}, {duration: periodValue / 2});
        if (voltageSliderValue) {
            piezoInterval = setInterval(function() {
                $(".gadget4 #miroirNoir2").velocity({translateY: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px"}, {duration: periodValue / 2});
                $(".straight-line-bottom").velocity({translateY: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px"}, {duration: periodValue / 2});
                $(".straight-line-bottom2").velocity({translateY: (-1) * (translateValue) + "px", translateX: (-1) * (translateValue) + "px"}, {duration: periodValue / 2}).velocity({translateY: 0 + "px", translateX: 0 + "px"}, {duration: periodValue / 2});
            }, periodValue);
    ...
    piezoVoltageSlider.noUiSlider.on('update', function(value){
        voltageSliderValue = parseFloat(value[0].replace('V', ''));
        translateValue = (w / 796) * (voltageSliderValue * 8) / (2 * 5);
        var logObject = {
            "objectType": "slider",
            "displayName":"piezoVoltageSlider",
            "content": voltageSliderValue
        }
        if(actionLoggerReady) 
            actionLogger.logChange(logObject);
        sendActuatorData('piezo', [voltageSliderValue, 1, frequencySliderValue]);
    });

    piezoVoltageSlider.noUiSlider.on('set', function(value){
        voltageSliderValue = parseFloat(value[0].replace('V', ''));
        initiatePiezoMovement();
    });

    var piezoFrequencySlider = document.getElementById('piezoFrequencySlider');
    noUiSlider.create(piezoFrequencySlider, {
        start: 0.1,
        connect: [true, false],
        orientation: 'vertical',
        direction: 'rtl',
        ...
        format: {
          to: function ( value ) {
            return (value!="0"?parseFloat(Math.round(value * 100) / 100).toFixed(2):0) + 'Hz';
          },
          from: function ( value ) {
    ...
    piezoFrequencySlider.noUiSlider.on('update', function(value){
        frequencySliderValue = parseFloat(value[0].replace('Hz', ''));
        periodValue = (1 / frequencySliderValue) * 1000;
        initiatePiezoMovement();
        var logObject = {
            "objectType": "slider",
            "displayName":"piezoFrequencySlider",
            "content": frequencySliderValue
        }
        if(actionLoggerReady) 
            actionLogger.logChange(logObject);
        sendActuatorData('piezo', [voltageSliderValue, 1, frequencySliderValue]);
    ...
    piezoFrequencySlider.noUiSlider.on('change', function(value){
        initiatePiezoMovement();
    });
    ...
    function createChart() {
        ...
        var updateChart = function() {
            ...
            newRow.c.push({v: voltageSliderValue, f: voltageSliderValue});
            newRow.c.push({v: 1, f: 1});
            newRow.c.push({v: frequencySliderValue, f: frequencySliderValue});
    ...
    window.onbeforeunload = function() {
        if (controller) 
            configureSD(0, 0, 0, [0, 1, 0.1], [0, enableCamera1]);
    }
