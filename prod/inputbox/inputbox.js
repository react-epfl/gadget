                ...
                .then(function(resource){
                    ...
                    $.when(create_data_vault(storageInfo.storageHandler))
                        .then(function(id){
                            ...
                            $.when(migrate_from_appdata())
                             .done(function(text){
                                    document.getElementById('scratchpad_text').value=text;
                                    save_data_vault();
                                    debuglogger("Migration successfully completed");
                                })
                             .fail(function(){
                                    debuglogger("Migration failed");
                                })
                             .always(function(){
                                    set_autosave();
                                    $("#scratchpad_text").prop('disabled', false);
...
function teacherButtonActions(){
    $("#button_select").fadeIn(500,function(){
        adjustHeight();
    });
    $("body")
    .on("click","#teacherViewButton",function(){
         $("#studentViewButton").removeClass("active");
         $(this).addClass("active");
         $("#scratchpad_text").fadeOut(200,function(){
            $("#scratchpad_input_teacher").fadeIn(200,function() {
                adjustHeight();
            });
        });
    })
    .on("click","#studentViewButton", function(){
        $("#teacherViewButton").removeClass("active");
        $(this).addClass("active");
        $("#scratchpad_input_teacher").fadeOut(200,function(){
            $("#scratchpad_text").fadeIn(200,function() {
                adjustHeight();
            });
    })
})
...
var displayStudentResults=function(resourceList){
    ...
    var inputTableHeader=$('<thead><tr><td>Student Name</td><td>Input</td><td>Date Added</td></tr>');
...
function debuglogger(msg) {                     //Custom logger for debugging purposes
    if (debug){
        var prefix="Scratchpad App "+appId+": ";
